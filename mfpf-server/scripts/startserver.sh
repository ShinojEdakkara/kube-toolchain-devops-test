...
usage() 
{
   ...
   echo "					   Example: wl.analytics.url:http://127.0.0.1/analytics-service/rest/data,wl.analytics.console.url:http://127.0.0.1/analytics/console" 
   echo 
...
then
    export ARGS_PROPS_FILE="startserver.properties"
    recordInputParams SERVER_IMAGE_TAG=$SERVER_IMAGE_TAG SERVER_CONTAINER_NAME=$SERVER_CONTAINER_NAME SERVER_MEM=$SERVER_MEM SERVER_IP=$SERVER_IP SERVICE_INSTANCES=$SERVICE_INSTANCES EXPOSE_HTTP=$EXPOSE_HTTP EXPOSE_HTTPS=$EXPOSE_HTTPS SSH_ENABLE=$SSH_ENABLE ENABLE_VOLUME=$ENABLE_VOLUME SSH_KEY=$SSH_KEY TRACE_SPEC=$TRACE_SPEC MAX_LOG_FILES=$MAX_LOG_FILES MAX_LOG_FILE_SIZE=$MAX_LOG_FILE_SIZE MAX_LOG_FILE_SIZE=$MAX_LOG_FILE_SIZE MFPF_PROPERTIES=$MFPF_PROPERTIES
fi

cmd="cf ic run --name $SERVER_CONTAINER_NAME -m $SERVER_MEM -e MF_REGION=$(getDetails Container_${SERVER_CONTAINER_NAME})"
if [ "$SSH_ENABLE" = "Y" ] || [ "$SSH_ENABLE" = "y" ]
then
