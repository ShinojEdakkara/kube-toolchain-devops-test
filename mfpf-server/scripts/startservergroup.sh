...
then
    export ARGS_PROPS_FILE="startservergroup.properties"
    recordInputParams SERVER_IMAGE_TAG=$SERVER_IMAGE_TAG SERVER_CONTAINER_GROUP_NAME=$SERVER_CONTAINER_GROUP_NAME SERVER_CONTAINER_GROUP_HOST=$SERVER_CONTAINER_GROUP_HOST SERVER_CONTAINER_GROUP_DOMAIN=$SERVER_CONTAINER_GROUP_DOMAIN SERVER_CONTAINER_GROUP_MIN=$SERVER_CONTAINER_GROUP_MIN SERVER_CONTAINER_GROUP_MAX=$SERVER_CONTAINER_GROUP_MAX SERVER_CONTAINER_GROUP_DESIRED=$SERVER_CONTAINER_GROUP_DESIRED ENABLE_AUTORECOVERY=$ENABLE_AUTORECOVERY SERVICE_INSTANCES=$SERVICE_INSTANCES SERVER_MEM=$SERVER_MEM TRACE_SPEC=$TRACE_SPEC MAX_LOG_FILES=$MAX_LOG_FILES MAX_LOG_FILE_SIZE=$MAX_LOG_FILE_SIZE ENABLE_VOLUME=$ENABLE_VOLUME MFPF_PROPERTIES=$MFPF_PROPERTIES
fi

cmd="cf ic group create --name $SERVER_CONTAINER_GROUP_NAME -n $SERVER_CONTAINER_GROUP_HOST -d $SERVER_CONTAINER_GROUP_DOMAIN -m $SERVER_MEM -e MFPF_CLUSTER_MODE=Farm --min $SERVER_CONTAINER_GROUP_MIN --max $SERVER_CONTAINER_GROUP_MAX --desired $SERVER_CONTAINER_GROUP_DESIRED  -e MF_REGION=$(getDetails CG_${SERVER_CONTAINER_GROUP_NAME} )"
...
then
    SERVER_CONTAINER_GROUP_ID=$(echo $CMD_RUN_RESULT | sed -e 's/.*([iI][dD]:\(.*\)).*/\1/' | tr -d " ")
else 
    echo "ERROR: cf ic group create command failed. Exiting ..."
    exit 1
fi
