...
usage() 
{
   ...
   echo "     -t | --tag ANALYTICS_IMAGE_TAG  Name of the MobileFirst analytics image."
...
readParams()
{
   ...
   #-------------------------------------
   INPUT_MSG="Specify the name to be used for the analytics image. Should be of form registryUrl/repositoryNamespace/imagename (mandatory) : "
   ERROR_MSG="Name for analytics image cannot be empty. Specify the name to be used for the analytics image (mandatory) : "
...
then
    export ARGS_PROPS_FILE="prepareanalytics.properties"
    recordInputParams ANALYTICS_IMAGE_TAG=$ANALYTICS_IMAGE_TAG
fi
