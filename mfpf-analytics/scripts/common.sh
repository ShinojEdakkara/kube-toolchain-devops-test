...
recordInputParams()
{
    ...
    then
        ...
        for oneParam in $*
        do
            oneParamName=$(echo ${oneParam} | cut -d"=" -f1 )
            #sed -e "s#^.*${oneParamName}=.*\$#${oneParam}#g" ${NEW_PROPS_FILE} > ${NEW_PROPS_FILE}
            sed -i.bak "s#^.*${oneParamName}=.*\$#${oneParam}#g" ${NEW_PROPS_FILE}
        done
        rm ${NEW_PROPS_FILE}.bak
    fi
