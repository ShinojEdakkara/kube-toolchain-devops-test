...
then
    export ARGS_PROPS_FILE="startanalyticsgroup.properties"
    recordInputParams ANALYTICS_IMAGE_TAG=$ANALYTICS_IMAGE_TAG ANALYTICS_CONTAINER_GROUP_NAME=$ANALYTICS_CONTAINER_GROUP_NAME ANALYTICS_CONTAINER_GROUP_MIN=$ANALYTICS_CONTAINER_GROUP_MIN ANALYTICS_CONTAINER_GROUP_MAX=$ANALYTICS_CONTAINER_GROUP_MAX ANALYTICS_CONTAINER_GROUP_DESIRED=$ANALYTICS_CONTAINER_GROUP_DESIRED ENABLE_AUTORECOVERY=$ENABLE_AUTORECOVERY ANALYTICS_CONTAINER_GROUP_HOST=$ANALYTICS_CONTAINER_GROUP_HOST ANALYTICS_CONTAINER_GROUP_DOMAIN=$ANALYTICS_CONTAINER_GROUP_DOMAIN SERVER_MEM=$SERVER_MEM TRACE_SPEC=$TRACE_SPEC MAX_LOG_FILES=$MAX_LOG_FILES MAX_LOG_FILE_SIZE=$MAX_LOG_FILE_SIZE MFPF_PROPERTIES=$MFPF_PROPERTIES ENABLE_VOLUME=$ENABLE_VOLUME ANALYTICS_DATA_VOLUME_NAME=$ANALYTICS_DATA_VOLUME_NAME ANALYTICS_DATA_DIRECTORY=$ANALYTICS_DATA_DIRECTORY
fi

cmd="cf ic group create --name $ANALYTICS_CONTAINER_GROUP_NAME -n $ANALYTICS_CONTAINER_GROUP_HOST -d $ANALYTICS_CONTAINER_GROUP_DOMAIN -m $SERVER_MEM --min $ANALYTICS_CONTAINER_GROUP_MIN --max $ANALYTICS_CONTAINER_GROUP_MAX --desired $ANALYTICS_CONTAINER_GROUP_DESIRED -p $ANALYTICS_HTTPPORT -e ANALYTICS_multicast=true"
...
createDataVolume
cmd="$cmd --volume $ANALYTICS_DATA_VOLUME_NAME:$ANALYTICS_DATA_DIRECTORY -e ANALYTICS_DATA_DIRECTORY=$ANALYTICS_DATA_DIRECTORY"
...
ANALYTICS_CLUSTER_NAME=mfpf_analytics_cluster_$ANALYTICS_CONTAINER_GROUP_NAME

NUMBER_OF_SHARDS=$ANALYTICS_CONTAINER_GROUP_MAX

cmd="$cmd -e ANALYTICS_TRACE_LEVEL=$TRACE_SPEC -e ANALYTICS_MAX_LOG_FILES=$MAX_LOG_FILES -e ANALYTICS_MAX_LOG_FILE_SIZE=$MAX_LOG_FILE_SIZE -e ANALYTICS_CLUSTER_NAME=$ANALYTICS_CLUSTER_NAME -e NUMBER_OF_SHARDS=$NUMBER_OF_SHARDS"
...
CREATE_STATUS=$(echo $CMD_RUN_RESULT | grep -i "The container group \"$ANALYTICS_CONTAINER_GROUP_NAME\"" | wc -l | tr -s " ")
if [ $(echo $CREATE_STATUS) != "0" ]
then
    ANALYTICS_CONTAINER_GROUP_ID=$(echo $CMD_RUN_RESULT | sed -e 's/.*([iI][dD]:\(.*\)).*/\1/' | tr -d " ")
else
    echo "ERROR: cf ic group create command failed. Exiting ..."
    exit 1
fi
