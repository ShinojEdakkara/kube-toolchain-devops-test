...
export CUSTOMIZATION_DIR=usr-mfpf-server
...
setDBInPropFile()
{
    sed -i.bak "s#MFPF_${1}_DB2_SERVER_NAME=.*\$#MFPF_${1}_DB2_SERVER_NAME=$(getSqldbHost)#g" ${KUBE_PROPERTY_FILE}
    sed -i.bak "s#MFPF_${1}_DB2_PORT=.*\$#MFPF_${1}_DB2_PORT=$(getSqldbPort)#g" ${KUBE_PROPERTY_FILE}
    sed -i.bak "s#MFPF_${1}_DB2_DATABASE_NAME=.*\$#MFPF_${1}_DB2_DATABASE_NAME=$(getSqldbName)#g" ${KUBE_PROPERTY_FILE}
    sed -i.bak "s#MFPF_${1}_DB2_USERNAME=.*\$#MFPF_${1}_DB2_USERNAME=$(getSqldbUsername)#g" ${KUBE_PROPERTY_FILE}
    sed -i.bak "s#MFPF_${1}_DB2_PASSWORD=.*\$#MFPF_${1}_DB2_PASSWORD=$(getSqldbPassword)#g" ${KUBE_PROPERTY_FILE}
    sed -i.bak "s#MFPF_${1}_DB2_SCHEMA=.*\$#MFPF_${1}_DB2_SCHEMA=${ADMIN_SCHEMA_NAME}#g" ${KUBE_PROPERTY_FILE}
    if [[ "${DB_TYPE}" = "DB2" ]]
    then
...
configureSqldb()
{
    ...
    export KUBE_PROPERTY_FILE=./args/mfpf-deployment-all.yaml
    setDBInPropFile ADMIN
    export KUBE_PROPERTY_FILE=./args/mfpf-deployment-all-with-tls.yaml
    setDBInPropFile ADMIN
    export KUBE_PROPERTY_FILE=./args/mfpf-deployment-server.yaml
    setDBInPropFile ADMIN
    export KUBE_PROPERTY_FILE=./args/mfpf-deployment-lite-server.yaml
    setDBInPropFile ADMIN
	...
	mv ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml-moved
	cat ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml-moved | grep -v "AUTO GENERATED LINE by prepareserverdbs.sh. DO NOT EDIT" > ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml
	rm ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml-moved
        if [ "$ENABLE_PUSH" = "Y" ]
        then
	        if [[ "${DB_TYPE}" = "DB2" ]]
	        then
            ...
            mv ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml-moved
            cat ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml-moved | grep -v "AUTO GENERATED LINE by prepareserverdbs.sh for push service. DO NOT EDIT" > ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml
	        rm ../$CUSTOMIZATION_DIR/config/mfpfsqldb.xml-moved
