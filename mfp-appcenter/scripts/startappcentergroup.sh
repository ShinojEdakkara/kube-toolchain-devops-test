...
{
   echo 
   echo " Running a MobileFirst Platform Foundation Platform Application Center Image as a Container Group "
   echo " --------------------------------------------------------------------------------------- "
   echo " This script runs the MobileFirst Platform Application Center image as a container group"
   ...
   echo "     -t | --tag SERVER_IMAGE_TAG                  Name of the MobileFirst Platform Application Center image available in the Bluemix registry"
   echo "     -gn | --name SERVER_CONTAINER_GROUP_NAME     Name of the MobileFirst Platform Application Center container group"
   ...
   echo "     -gd | --desired SERVER_CONTAINER_GROUP_DESIRED (Optional) The desired number of instances. The default value is 1"
   echo "     -a  | --auto ENABLE_AUTORECOVERY                Enable Auto-recovery option for the container instances. The default is N"
   echo "     -si | --services SERVICE_INSTANCES           (Optional) Comma-separated Bluemix service instance names that should be bound to the container"
   echo "     -tr | --trace TRACE_SPEC             (Optional) Trace specification to be applied to MobileFirst Platform Application Center"
...
readParams()
{
      ...
      # Read the name of the MobileFirst Platform Application Center image
      #-------------------------------------------------------------------
      INPUT_MSG="Specify the name of the MobileFirst Platform Application Center image available in the Bluemix registry. Should be of form registryUrl/repositoryNamespace/imagename (mandatory) : "
      ERROR_MSG="Name of the server image cannot be empty. Specify the name of the MobileFirst Platform Application Center image. Should be of form registryUrl/repositoryNamespace/imagename (mandatory) : "
      SERVER_IMAGE_TAG=$(fnReadInput "$INPUT_MSG" "$ERROR_MSG")

      # Read the name of the container 
      #-------------------------------
      INPUT_MSG="Specify the name for the MobileFirst Platform Application Center container group (mandatory) : "
      ERROR_MSG="Container group name cannot be empty. Specify the name for the MobileFirst Platform Application Center container group (mandatory) : "
      ...
      #-------------------------------------
      INPUT_MSG="Specify the number of instances to create. The default value is 1 (optional) : "
      ERROR_MSG="Error due to non-numeric input. Specify the number of instances to create. The default value is 2 (optional) : "
      SERVER_CONTAINER_GROUP_DESIRED=$(fnReadNumericInput "$INPUT_MSG" "$ERROR_MSG" "1")
      ...
      ERROR_MSG="Input should be either Y or N. Enable autorecovery for the MobileFirst Platform Application Center container instances. Accepted values are Y or N. The default value is N (optional) : "
	  ...
	  # Read the Bluemix service instance names
      #----------------------------------------
      read -p "Specify the Bluemix service instance names that should be bound to the container in a comma-separated manner (optional) : " SERVICE_INSTANCES
	  
      # Read the memory for the MobileFirst Server container
      #-----------------------------------------------------
      INPUT_MSG="Specify the memory size limit (in MB) for the MobileFirst Platform Application Center container. Accepted values are 1024, 2048,... The default value is 1024 (optional) : "
      ...
      ERROR_MSG="Input should be either Y or N. Enable mounting volume for the MobileFirst Platform Application Center container logs. Accepted values are Y or N. The default value is N (optional) : "
      ENABLE_VOLUME=$(readBoolean "$INPUT_MSG" "$ERROR_MSG" "N")
      
      read -p "Provide the Trace specification to be applied to the MobileFirst Platform Application Center. The default value is *=info (optional) : " TRACE_SPEC
...
validateParams() 
{
   ...
   then 
		SERVER_CONTAINER_GROUP_DESIRED=1;
   fi
...
cmd="cf ic group create --name $SERVER_CONTAINER_GROUP_NAME -n $SERVER_CONTAINER_GROUP_HOST -d $SERVER_CONTAINER_GROUP_DOMAIN -m $SERVER_MEM --min $SERVER_CONTAINER_GROUP_MIN --max $SERVER_CONTAINER_GROUP_MAX --desired $SERVER_CONTAINER_GROUP_DESIRED"
...
then
   createVolumes
   cmd="$cmd --volume $SYSVOL_NAME:$SYSVOL_PATH"
   cmd="$cmd --volume $LIBERTYVOL_NAME:$LIBERTYVOL_PATH"
...
then
   cmd="$cmd -p $APPCENTER_SERVER_HTTPPORT"
fi

if [ ! -z "$SERVICE_INSTANCES" ]
then 
   cmd="$cmd -e CCS_BIND_SRV=$SERVICE_INSTANCES"
fi
...
echo "Starting the MobileFirst Platform Application Center container group : " $SERVER_CONTAINER_GROUP_NAME
...
CREATE_STATUS=$(echo $CMD_RUN_RESULT | grep -i "The container group \"$SERVER_CONTAINER_GROUP_NAME\"" | wc -l | tr -s " ")
