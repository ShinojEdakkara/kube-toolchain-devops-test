...
{
   echo 
   echo " Running a MobileFirst Platform Application Center Image as a Container "
   echo " --------------------------------------------------------------------------------- "
   echo " This script runs the MobileFirst Platform Application Center image as a container"
   ...
   echo "     -t | --tag SERVER_IMAGE_TAG        Name of the MobileFirst Platform Application Center image available in the registry"
   echo "     -n | --name SERVER_CONTAINER_NAME  Name of the MobileFirst Server container"
   echo "     -i | --ip SERVER_IP                IP address that the MobileFirst Platform Application Center container should be bound to."
...
readParams()
{
   ...
   #----------------------------------------------
   INPUT_MSG="Specify the name of the MobileFirst Platform Application Center image available in the Bluemix registry. Should be of form registryUrl/repositoryNamespace/name (mandatory) : "
   ERROR_MSG="Name of the MobileFirst Server image cannot be empty. Specify the name of the MobileFirst Platform Application Center image available in the Bluemix registry (mandatory) : "
   ...
   #---------------------------------------------------------
   INPUT_MSG="Specify the IP address for the MobileFirst Platform Application Center container (mandatory) : "
   ERROR_MSG="Incorrect IP address. Specify the correct IP address for the MobileFirst Platform Application Center container (mandatory) : " 
   ...
   # Read the memory size for the MobileFirst Platform Application Center container
   #----------------------------------------------------------
   ...
   #------------------------------------------------
   INPUT_MSG="Enable SSH For the MobileFirst Platform Application Center container. Accepted values are Y or N. The default value is Y. (optional) : "
   ERROR_MSG="Input should be either Y or N. Enable SSH For the MobileFirst Platform Application Center container. Accepted values are Y or N. The default value is Y. (optional) : "
   ...
   INPUT_MSG="Enable mounting volume for the MobileFirst Platform Application Center container logs. Accepted values are Y or N. The default value is N (optional) : " 
...
then
   cmd="$cmd -p $SERVER_IP:$APPCENTER_SERVER_HTTPPORT:$APPCENTER_SERVER_HTTPPORT"
fi

if [ "$EXPOSE_HTTPS" = "Y" ] || [ "$EXPOSE_HTTPS" = "y" ]
then
	cmd="$cmd -p $SERVER_IP:$APPCENTER_SERVER_HTTPSPORT:$APPCENTER_SERVER_HTTPSPORT"
fi
...
echo "Starting the MobileFirst Platform Application Center container : " $SERVER_CONTAINER_NAME
